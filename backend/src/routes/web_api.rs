use axum::{ response::Html, routing::{ get, post } };
use tower_http::cors::{Any, CorsLayer};
use crate::{ adapters::gemini_response::GeminiQueryProvider, handlers::{ self }, types::AppState };
use http::header::{ACCEPT, ACCESS_CONTROL_ALLOW_ORIGIN, AUTHORIZATION, CONTENT_TYPE};
use http::Method;

pub async fn web_api() -> axum::Router {
    let ai_chat_provider = GeminiQueryProvider::new(
